...
def render_part1_images(work_dir: Path) -> list[ChartImage]:
    ...
    buy_minus_sell_df = normalize_dates(read_ods_sheet(CALL_PUT_ODS, "買權減賣權"))
    ...
    buy_minus_sell_df = to_numeric_series(
        buy_minus_sell_df,
        ["未平倉金額_自營商", "外資"],
    ).sort_values("日期").reset_index(drop=True)
    if buy_minus_sell_df.empty:
        raise RuntimeError("Sheet 買權減賣權 has no usable rows in call_put_capital_flow.ods")
...
def main() -> int:
    ...
    try:
        ...
        alert = f"[發送失敗] send_figure_everyday_thru_Line_V2 | error={exc}"
